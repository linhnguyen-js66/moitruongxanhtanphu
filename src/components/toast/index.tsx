...
function handleToastIcon(props: ToastProps) {
  if (props.icon) {
    ...
    //   height={props.icon.height} width={props.icon.width}
    // />
  }

  switch (props.type) {
    case TYPE_TOAST.INFO:
      return null; // <IconSvgLocal name="ICON_INFO" fill="rgb(var(--link-600))" height={24} width={24} />;
    case TYPE_TOAST.SUCCESS:
      return null; // <IconSvgLocal name="ICON_CHECK" fill="rgb(var(--success-600))" height={24} width={24} />
    case TYPE_TOAST.WARNING:
      return null;
    // <IconSvgLocal name="ICON_WARNING" fill="rgb(var(--pending-600))" height={24} width={24} />
    case TYPE_TOAST.ERROR:
      return null; // <IconSvgLocal name="ICON_CLOSE" fill="rgb(var(--error-600))" height={24} width={24} />;
    default:
      return null;
  }
}
